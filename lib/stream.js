...
Stream.prototype.process =  function process(data, offset) {
  ...
  if (this.size === 0) {
    ...
    moreResult = this.process(data, offset + msgSize);
    ...
    msgSize = this.buffer.readUInt16BE(offset + ofp.offsets.ofp_header.length, true);
    ...
    /* Note: recursive call, internal data will be changed */
    moreResult = this.process(this.EMPTY_BUFFER, 0);
    return result.concat(moreResult);
  }
};
