...
module.exports = {
  ...
  "unpack" : function(buffer, offset) {
    ...
    switch (type) {
      case ofp.ofp_instruction_type.OFPIT_GOTO_TABLE:
        return gotoTable.unpack(buffer, offset);
      case ofp.ofp_instruction_type.OFPIT_WRITE_METADATA:
        return writeMetadata.unpack(buffer, offset);
      case ofp.ofp_instruction_type.OFPIT_WRITE_ACTIONS:
        return writeActions.unpack(buffer, offset);
      case ofp.ofp_instruction_type.OFPIT_APPLY_ACTIONS:
        return applyActions.unpack(buffer, offset);
      case ofp.ofp_instruction_type.OFPIT_CLEAR_ACTIONS:
        return clearActions.unpack(buffer, offset);
  ...
  "pack" : function(instruction, buffer, offset) {
    if (buffer.length < offset + ofp.sizes.ofp_instruction) {
      return {
        error: {
          desc: util.format('instruction at offset %d does not fit the buffer.', offset)
        }
      };
    }
    ...
    switch (instruction.header.type) {
      case 'OFPIT_GOTO_TABLE':
        return gotoTable.pack(instruction, buffer, offset);
      case 'OFPIT_WRITE_METADATA': 
        return writeMetadata.pack(instruction, buffer, offset);
      case 'OFPIT_WRITE_ACTIONS': 
        return writeActions.pack(instruction, buffer, offset);
      case 'OFPIT_APPLY_ACTIONS': 
        return applyActions.pack(instruction, buffer, offset);
      case 'OFPIT_CLEAR_ACTIONS':
        return clearActions.pack(instruction, buffer, offset);
      case 'OFPIT_EXPERIMENTER': 
        return experimenter.pack(instruction, buffer, offset);
      default: {
        return {
          "error": {
